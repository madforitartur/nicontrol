...
interface OrdersContextType {
  ...
  importOrders: (newOrders: Order[], filename: string, errors?: number) => void;
...
export const OrdersProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const importOrders = useCallback((newOrders: Order[], filename: string, errors?: number) => {
    ...
    const historyItem: ImportHistoryItem = {
      ...
      filename,
      date: dateStr,
      records: newOrders.length,
      status: errors ? (errors > 0 ? 'partial' : 'success') : 'success',
      errors,
    };
    
    setImportHistory(prev => [historyItem, ...prev.slice(0, 9)]); // Keep last 10
    setIsLoading(false);
  }, []);
